 ...
 *
 */
public abstract class VerticalGalleryAdapterView<T extends Adapter> extends
		ViewGroup {
	...
	public VerticalGalleryAdapterView(Context context) {
		super(context);
	}
	...
	public interface OnItemClickListener {
		 ...
		 */
		void onItemClick(VerticalGalleryAdapterView<?> parent, View view,
	...
	public interface OnItemLongClickListener {
		 ...
		 */
		boolean onItemLongClick(VerticalGallery verticalGallery, View view,
				int position, long id);
	}
	...
	public interface OnItemSelectedListener {
		 ...
		 */
		void onItemSelected(VerticalGalleryAdapterView<?> parent, View view,
				int position, long id);
		 ...
		 */
		void onNothingSelected(VerticalGalleryAdapterView<?> parent);
	}
	 ...
	 * view
	 * call the empty view. The empty view is used to provide feedback to the
	 * user
	 ...
	 * true and
	 * we have an empty view, display it. In all the other cases, make sure that
	 * the listview
	...
	private void updateEmptyStatus(boolean empty) {
		...
		if (empty) {
			...
			if (mDataChanged) {
				this.layout(getLeft(), getTop(), getRight(),
						getBottom());
			}
		} else {
			if (mEmptyView != null)
	...
	class AdapterDataSetObserver extends DataSetObserver {
		...
		public void onChanged() {
			...
			if (VerticalGalleryAdapterView.this.getAdapter().hasStableIds()
					...
					&& mItemCount > 0) {
				VerticalGalleryAdapterView.this
		...
		public void onInvalidated() {
			...
			if (VerticalGalleryAdapterView.this.getAdapter().hasStableIds()) {
				...
				mInstanceState = VerticalGalleryAdapterView.this
						.onSaveInstanceState();
			}
	 ...
	 * starts at mSyncPosition
	 * and then alternates between moving up and moving down until 1) we find
	 * the right position, or
	 * 2) we run out of time, or 3) we have looked at every position
	 *
	 * @return Position of the row that matches mSyncRowId, or
	 *         {@link #INVALID_POSITION} if it can't
	 *         be found
	 */
