...
/**
 * <a href="http://beefsalt.blog79.fc2.com/">Ö÷¤ËAndroid</a>
 * <br/>
 * <a href="http://www.apache.org/licenses/LICENSE-2.0">http://www.apache.org/licenses/LICENSE-2.0</a>
 * @author beefsalt
 *
 */
public class VerticalGallery extends VerticalGalleryAbsSpinner implements
		GestureDetector.OnGestureListener {

	private static final String TAG = "VerticalGallery";
	...
	private void init(Context context){
		...
		View view = mContext.getLayoutInflater().inflate(R.layout.activity_choose, null);
        addView(view);
...
//		setSpacing(0);
	...
	void trackMotionScroll(int deltaY) {
		if(localLOGV)Log.d(TAG, "trackMotionScroll: "+deltaY);
		if (getChildCount() == 0) {
			return;
		}
...
//		return view.getLeft() + view.getWidth() / 2;
	...
	private void scrollIntoSlots() {
		...
		if(localLOGV)Log.d(TAG, "targetCenter="+targetCenter+",selectedCenter="+selectedCenter+",scrollAmount="+scrollAmount);
		if (scrollAmount != 0) {
	...
	private void setSelectionToCenterChild() {
		...
		for (int i = getChildCount() - 1; i >= 0; i--) {
			...
			int childClosestEdgeDistance = Math.min(Math.abs(child.getTop()
					- galleryCenter), Math
	...
	void layout(int delta, boolean animate) {
		...
		int selectedOffset = childrenTop + (childrenHeight / 2)
				- (sel.getHeight() / 2);
//		sel.offsetLeftAndRight(selectedOffset);
	...
	private void setUpChild(View child, int offset, int y, boolean fromTop) {
		...
		// make some up...
		VerticalGallery.LayoutParams lp = (VerticalGallery.LayoutParams) child
				.getLayoutParams();
		if (lp == null) {
			lp = (VerticalGallery.LayoutParams) generateDefaultLayoutParams();
		}
		...
		// Position horizonally based on gravity setting
	...
	public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX,
		...
		if (localLOGV)
			Log.v(TAG, String.valueOf(e2.getY() - e1.getY()));
...
//		return event.dispatch(this, null, null);
	...
	private class FlingRunnable implements Runnable {
		...
		public void startUsingVelocity(int initialVelocity) {
			if(localLOGV)Log.i(TAG, "startUsingVelocity="+initialVelocity);
		...
		public void run() {
			...
			if (delta > 0) {
				...
				delta = Math.min(getHeight() - getPaddingTop() - getPaddingBottom() - 1,
						delta);
			} else {
				...
				// Don't fling more than 1 screen
				delta = Math
						.max(-(getHeight() - getPaddingBottom() - getPaddingTop() - 1),
								delta);
			}

			trackMotionScroll(delta);
			if(localLOGV)Log.d(TAG, "mLastFlingY="+mLastFlingY+", delta="+delta);
